...
router.patch("/:id", async(req, res, next) => {
  ...
  if(dirData !== user.id){
    res.status(403).json({message:"You are not authorized to rename this directory!"})
  }
...
router.delete("/:id", async (req, res, next) => {
  ...
  if(directoryData.userId !== user.id){
    res.status(403).json({message: "You are not authorized to delete this directory!" })
  }
